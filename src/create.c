...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/07 15:44:07 by rafael-m          #+#    #+#             */
/*   Updated: 2025/12/07 16:34:43 by rafael-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_init_philos(
	t_philo **philos, const t_table *table, t_mtxs *mtxs, int *dead)
{
	int	i;
...
int	ft_init_mtxs(t_mtxs *mtxs, const int n_philos)
{
	...
	mtxs->forks = ft_calloc(n_philos, sizeof(pthread_mutex_t *));
	i = 0;
	while (mtxs->forks && i < n_philos)
		mtxs->forks[i++] = NULL;
	if (!mtxs->dead_m || !mtxs->printer || !mtxs
		->last_meal_mtx || !mtxs->forks || !mtxs->finished_mtx)
